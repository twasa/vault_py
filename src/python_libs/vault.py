...
import jmespath
from python_libs import config, jlogger, tools_datetime, tools_http
...
class Vault:
    ...
    def __init__(self):
        ...
        self.token = ""
        self.token_expire_timestamp = 0
        self.role = ""
        self.policies = []
        self.lease_duration = 0

    def info(self):
        return {"uri": self.vault_uri, "role": self.role, "policies": self.policies}
    ...
    def auth_k8s(self):
        ...
        if not r or r.status_code != 200:
            ...
            return
        response_data = json.loads(r.content)
        lease_duration = jmespath.search("auth.lease_duration", response_data)
        self.token_expire_timestamp = (
            tools_datetime.unix_time_now_int() + lease_duration
        )
    ...
    def sys_status(self) -> bool:
        ...
        headers = {"X-Vault-Request": "true", "X-Vault-Token": self.token}
        try:
    ...
    def token_check(self):
        if tools_datetime.unix_time_now_int() >= self.token_expire_timestamp:
            self.token = ""
        if not self.token:
            self.login()
